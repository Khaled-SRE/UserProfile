...
class UserProfileDeleteImageTest extends TestCase
{
    use RefreshDatabase;

    const USER_PROFILE_STORE_OR_UPDATE_IMAGE_ROUTE = 'user-profile.image.store';

    const USER_PROFILE_DELETE_IMAGE_ROUTE = 'user-profile.image.destroy';

    public function test_delete_user_profile_image_successfully(): void
    {
        Storage::fake('public');
        ...
        $response = $this->deleteJson(route(self::USER_PROFILE_DELETE_IMAGE_ROUTE, ['userId' => $userProfile->user_id]));
        $response->assertOk();
        $response->assertJson([
            'status' => true,
